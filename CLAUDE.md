...
## Recent decisions
- DB hardening (July 2026, applied as migrations on MAIN): RLS enabled on 30 previously-unprotected shared tables (with behavior-preserving `<table>_allow_all` policies — tighten per app later), duplicate permissive policies dropped on `activities`/`employees`, 9 unused indexes dropped on `todos`/`locations`/`task_jobs`, the 4 `SECURITY DEFINER` views (`gear_v`, `trailers_v`, `vehicles_v`, `employee_course_completion`) switched to `security_invoker`, `search_path` pinned to `public, extensions, pg_temp` on all 26 flagged functions, and Postgres upgraded 17.4→17.6. Zero ERROR-level advisor findings remain; leftover WARN/INFO lints are deliberate design (open policies, public media buckets, anon-callable login RPCs) or non-actionable (`pg_net` is not relocatable)
